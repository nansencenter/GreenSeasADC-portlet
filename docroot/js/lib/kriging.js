...
var matrixmult = function(y, x) {
	if (debugElKriging) console.log("matrixmult");
	if (debugElKriging) console.log(y);
	if (debugElKriging) console.log(x);
	var i, j, k;
	var n = x.length;
	var m = x[0].length;
	if (m != y.length)
...
function kriging(id, pixelsize) {
	...
	/*
	 * Kriging method Usage: kriging(longitude, latitude, response, polygons)
	 */
	this.krig = function(x, y, response, polygons) {
		...
		/**
		 * Calculate the euclidean distance matrix for the coordinates and the
		 * outcome variable
		 */
		...
		/**
		 * Calculate the inverted (n+1) x (n+1) matrix Used to calculate weights
		 */
	...
	this.canvas.model.spherical = function(h) {
		if (h > this.range)
			return this.sill;
		if (h <= this.range && h > 0) {
			return this.nugget + (this.sill - this.nugget)
					* ((3 * h) / (2 * this.range) - Math.pow(h, 3) / (2 * Math.pow(this.range, 3)));
		} else
			return 0;
	}
	...
	this.canvas.model.pred = function(x, y) {
		if (debugElKriging) console.log("model.pred");
		if (debugElKriging) console.log(x);
		if (debugElKriging) console.log(y);
		var i;
	...
	this.map = function(center, zoom) {
		...
		/**
		 * Loop through the polygons to determine the limits based on the area
		 * of each of the polygons. AND Create an Array containing the center
		 * coordinates for each polygon to be used during the sorting algorithm.
		 */
		var i, j;
		...
		for (i = 0; i < this.canvas.polygons.length; i++) {
			if (i == 0) {
				...
				if (this.canvas.polygons[i][0].min() < this.canvas.xlim[0])
					this.canvas.xlim[0] = this.canvas.polygons[i][0].min();
				if (this.canvas.polygons[i][0].max() > this.canvas.xlim[1])
					this.canvas.xlim[1] = this.canvas.polygons[i][0].max();
			}
		...
		/**
		 * Calculate the ratio and pixel size for conversion between units.
		 */
		this.canvas.xratio = (this.canvas.xlim[1] - this.canvas.xlim[0]) / this.canvas.width;
		this.canvas.yratio = this.canvas.xratio * yxratio;

		this.canvas.xlim = [ center[0] - 0.5 * this.canvas.width * this.canvas.xratio,
				center[0] + 0.5 * this.canvas.width * this.canvas.xratio ];
		this.canvas.ylim = [ center[1] - 0.5 * this.canvas.height * this.canvas.yratio,
				center[1] + 0.5 * this.canvas.height * this.canvas.yratio ];
	...
	this.canvas.zoom = function(zoom, yxratio, pixelsize) {
		/* Re-size the limits */
		var newlen = [ zoom * (this.xlim[1] - this.xlim[0]) / 2, zoom * (this.ylim[1] - this.ylim[0]) / 2 ];
	...
	this.canvas.colorspectrum.heatcolors = [ "#FF0000", "#FF0700", "#FF0E00", "#FF1500", "#FF1C00", "#FF2200",
			"#FF2900", "#FF3000", "#FF3700", "#FF3E00", "#FF4500", "#FF4C00", "#FF5300", "#FF5A00", "#FF6000",
			"#FF6700", "#FF6E00", "#FF7500", "#FF7C00", "#FF8300", "#FF8A00", "#FF9100", "#FF9800", "#FF9F00",
			"#FFA500", "#FFAC00", "#FFB300", "#FFBA00", "#FFC100", "#FFC800", "#FFCF00", "#FFD600", "#FFDD00",
			"#FFE300", "#FFEA00", "#FFF100", "#FFF800", "#FFFF00", "#FFFF0B", "#FFFF20", "#FFFF35", "#FFFF4A",
			"#FFFF60", "#FFFF75", "#FFFF8A", "#FFFF9F", "#FFFFB5", "#FFFFCA", "#FFFFDF", "#FFFFF4" ];
	this.canvas.colorspectrum.terraincolors = [ "#00A600", "#07A800", "#0EAB00", "#16AE00", "#1DB000", "#25B300",
			"#2DB600", "#36B800", "#3EBB00", "#47BE00", "#50C000", "#59C300", "#63C600", "#6CC800", "#76CB00",
			"#80CE00", "#8BD000", "#95D300", "#A0D600", "#ABD800", "#B6DB00", "#C2DE00", "#CEE000", "#D9E300",
			"#E6E600", "#E6DD09", "#E7D612", "#E7CF1C", "#E8C825", "#E8C32E", "#E9BE38", "#E9BA41", "#EAB74B",
			"#EAB454", "#EBB25E", "#EBB167", "#ECB171", "#ECB17B", "#EDB285", "#EDB48E", "#EEB798", "#EEBAA2",
			"#EFBFAC", "#EFC4B6", "#F0C9C0", "#F0D0CA", "#F1D7D4", "#F1DFDE", "#F2E8E8", "#F2F2F2" ];
	this.canvas.colorspectrum.topocolors = [ "#4C00FF", "#3B00FF", "#2800FF", "#1600FF", "#0400FF", "#000DFF",
			"#001FFF", "#0032FF", "#0043FF", "#0055FF", "#0068FF", "#007AFF", "#008BFF", "#009EFF", "#00AFFF",
			"#00C1FF", "#00D3FF", "#00E5FF", "#00FF4D", "#00FF38", "#00FF24", "#00FF0F", "#05FF00", "#1AFF00",
			"#2EFF00", "#42FF00", "#57FF00", "#6BFF00", "#80FF00", "#94FF00", "#A8FF00", "#BDFF00", "#D1FF00",
			"#E6FF00", "#FFFF00", "#FFF90C", "#FFF318", "#FFED24", "#FFE930", "#FFE53B", "#FFE247", "#FFDF53",
			"#FFDD5F", "#FFDC6B", "#FFDB77", "#FFDB83", "#FFDB8F", "#FFDC9B", "#FFDEA7", "#FFE0B3" ];
	this.canvas.colorspectrum.cmcolors = [ "#80FFFF", "#85FFFF", "#8AFFFF", "#8FFFFF", "#94FFFF", "#99FFFF", "#9EFFFF",
			"#A3FFFF", "#A8FFFF", "#ADFFFF", "#B3FFFF", "#B8FFFF", "#BDFFFF", "#C2FFFF", "#C7FFFF", "#CCFFFF",
			"#D1FFFF", "#D6FFFF", "#DBFFFF", "#E0FFFF", "#E6FFFF", "#EBFFFF", "#F0FFFF", "#F5FFFF", "#FAFFFF",
			"#FFFAFF", "#FFF5FF", "#FFF0FF", "#FFEBFF", "#FFE6FF", "#FFE0FF", "#FFDBFF", "#FFD6FF", "#FFD1FF",
			"#FFCCFF", "#FFC7FF", "#FFC2FF", "#FFBDFF", "#FFB8FF", "#FFB3FF", "#FFADFF", "#FFA8FF", "#FFA3FF",
			"#FF9EFF", "#FF99FF", "#FF94FF", "#FF8FFF", "#FF8AFF", "#FF85FF", "#FF80FF" ];
	...
	this.canvas.pixel = function(x, y, col) {
		...
		/* Spaced-out pixels */
		// this.ctx.fillRect((x-this.xlim[0])/this.xratio - pixelsize/2 + 1,
		// this.height - (y-this.ylim[0])/this.yratio - pixelsize/2 + 1,
		// pixelsize - 2, pixelsize - 2);
		/* Solid map */
		this.ctx.fillRect((x - this.xlim[0]) / this.xratio - pixelsize / 2, this.height - (y - this.ylim[0])
				/ this.yratio - pixelsize / 2, pixelsize, pixelsize);
	}

	this.canvas.focus = function(x, y, col) {
		this.ctx.beginPath();
		this.ctx.arc((x - this.xlim[0]) / this.xratio, this.height - (y - this.ylim[0]) / this.yratio, 2, 0,
				2 * Math.PI, false);
		this.ctx.fillStyle = col;
		this.ctx.fill();
	}
	...
	this.canvas.background = function() {
		/**
		 * 1) Nearest-neighbor 2) Loop through sorted polygons 3)
		 * Point-in-polygon for eligible points 4) Break if no points in polygon
		 */

		var i, j, k;
		for (i = 0; i < this.polygoncenters.length; i++) {
			this.polygonsorted[i] = Math.sqrt(Math.pow(this.polygoncenters[i][0] - this.xlim.mean(), 2)
					+ Math.pow(this.polygoncenters[i][1] - this.ylim.mean(), 2));
		}
		...
		for (i = 0; i < this.polygons.length; i++) {
			if (debugElKriging) console.log("i:" + i);
			/* Calculate the intersecting box */
			if (this.xlim[0] > this.polygons[nearest][0].min())
				xbox[0] = this.xpixel * Math.floor(this.xlim[0] / this.xpixel);
			else
				xbox[0] = this.xpixel * Math.floor(this.polygons[nearest][0].min() / this.xpixel);

			if (this.xlim[1] < this.polygons[nearest][0].max())
				xbox[1] = this.xpixel * Math.ceil(this.xlim[1] / this.xpixel);
			else
				xbox[1] = this.xpixel * Math.ceil(this.polygons[nearest][0].max() / this.xpixel);

			if (this.ylim[0] > this.polygons[nearest][1].min())
				ybox[0] = this.ypixel * Math.floor(this.ylim[0] / this.ypixel);
			else
				ybox[0] = this.ypixel * Math.floor(this.polygons[nearest][1].min() / this.ypixel);

			if (this.ylim[0] < this.polygons[nearest][1].max())
				ybox[1] = this.ypixel * Math.ceil(this.ylim[1] / this.ypixel);
			else
				ybox[1] = this.ypixel * Math.ceil(this.polygons[nearest][1].max() / this.ypixel);
			...
			for (j = xbox[0]; j <= xbox[1]; j += this.xpixel) {
				for (k = ybox[0]; k <= ybox[1]; k += this.ypixel) {
					if (pip(this.polygons[nearest][0], this.polygons[nearest][1], j, k)) {
						color = Math.round(49 * (this.model.pred(j, k) - this.model.response_min)
								/ (this.model.response_range));
						if (color < 0)
							color = 0;
						else if (color > 49)
							color = 49;
