...
myNamespace.query = (function(OL, $) {
	...
	function filterToXmlString(filter) {
		...
		if (myNamespace.projectionCode != "EPSG:4326") {
			var index = writtenXML.indexOf('<gml:Polygon xmlns:gml="http://www.opengis.net/gml">') + 51;
			writtenXML = [ writtenXML.slice(0, index), ' srsName="' + myNamespace.projectionCode + '"',
					writtenXML.slice(index) ].join("");
		}
		return writtenXML;
	}
	...
	function createfilterBoxHashMap() {
		...
		if (document.getElementById('bboxEnabledCheck').checked) {
			...
			var linearRing = new OL.Geometry.LinearRing([ new OpenLayers.Geometry.Point(left, top),
					new OpenLayers.Geometry.Point(right, top), new OpenLayers.Geometry.Point(right, bottom),
					new OpenLayers.Geometry.Point(left, bottom), new OpenLayers.Geometry.Point(left, top) ]);
