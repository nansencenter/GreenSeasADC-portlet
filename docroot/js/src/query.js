...
myNamespace.query = (function(OL, $) {
	...
	function constructFilter(bbox, date, depth, param, months, region) {

		// should we filter at all?
		if (bbox || date || depth || param || months || region) {
			var filterArray = [];
	...
	function constructFilterString(bbox, date, attributes, depth, months, region) {
		if (debugq)
			console.log("constructFilterString");
		// generate filter object
		var filterObject = constructFilter(bbox, date, depth, null, months, region);

		return constructString(filterObject);

	}

	function constructParameterFilterString(parameters, depth, bbox, date, months, region) {
		if (debugq)
			console.log("constructParameterFilterString starting");
		// generate filter object
		var filterObject = constructFilter(bbox, date, depth, parameters, months, region);
